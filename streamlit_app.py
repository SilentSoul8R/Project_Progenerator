...
st.set_page_config(
    page_title="VisionCraft AI | Image Generator",
    page_icon="🎨",
    layout="wide",
...
# Priority 2: Streamlit Secrets (for deployed apps)
# Priority 3: Interactive sidebar input (if not pre-configured)
...
with st.sidebar:
    ...
    enhance_prompt = st.toggle(
        ...
        value=True,
        help="Use Groq Llama 3 to automatically expand simple ideas into ultra-detailed artistic prompts."
    )
...
def enhance_prompt_with_groq(user_prompt: str) -> str:
    """Uses Groq's high-speed Llama-3 model to craft detailed art prompts."""
    if not client:
        return user_prompt
    
    system_instruction = (
        "You are an expert AI image generation prompt engineer. "
        "Expand the user input into a highly detailed, vivid, descriptive image prompt. "
        "Include lighting, perspective, art style, color palette, and atmospheric details. "
        "Return ONLY the final detailed prompt, with no explanations or preamble."
    )
    ...
    try:
        ...
        st.warning(f"Could not reach Groq API for prompt enhancement: {e}")
        return user_prompt


def generate_image(prompt: str, w: int, h: int, seed_val: int):
    """Fetches high-quality artwork from Flux via Pollinations."""
    formatted_prompt = requests.utils.quote(prompt)
    url = f"https://image.pollinations.ai/prompt/{formatted_prompt}?width={w}&height={h}&seed={seed_val}&nologo=true&model=flux"
...
st.markdown('<div class="header-subtitle">Turn your imagination into breathtaking visual art using Groq & Flux.</div>', unsafe_allow_html=True)
...
with col1:
    ...
    user_prompt = st.text_area(
        "Prompt Input",
        placeholder="e.g., A majestic cybernetic owl perched on an ancient neon glowing tree in a misty forest...",
        height=160,
...
if generate_btn:
    if not user_prompt.strip():
        ...
        # Step 1: Prompt Enhancement using Groq
        if enhance_prompt:
            if not groq_api_key:
                st.error("Groq API key is missing. Add it to `.env` or the sidebar to use Magic Enhance.")
                st.stop()
            
            with st.status("🧠 Groq LLM is crafting your detailed visual prompt...", expanded=False) as status:
                final_prompt = enhance_prompt_with_groq(user_prompt)
                status.update(label="✨ Prompt Enhanced Successfully!", state="complete")
            
            st.caption(f"**Enhanced Visual Prompt:** _{final_prompt}_")
        ...
        with st.spinner("🎨 Rendering high-resolution artwork..."):
            try:
                ...
                with col2:
                    ...
                    # Convert PIL image to byte stream for direct download
                    buf = io.BytesIO()
